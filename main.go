...
func (p *Product) SignUp(c *gin.Context) {
	...
	creds1 := &Credentials{Username: creds.Username, Password: hash(creds.Password)}
	if err != nil {
...
func (p *Product) newPasswordToken(c *gin.Context) {
	...
	er := p.Db.Model(&item).Where("username = ?", us).Update("password", hash(pw)).Error
	if er != nil {
